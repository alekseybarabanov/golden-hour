...
```
и отправить пользователю только поле `portal_url` из JSON. Никогда не повторять старую ссылку из истории, памяти или предыдущего ответа. `portal_url` — это адрес хоста, на котором работает агент (Pi/ПК) в локальной сети: телефон должен быть в **той же сети/Wi‑Fi**, что и этот хост. Поле `mode` в JSON подсказывает способ доступа (`lan` — общий Wi‑Fi/роутер, `hotspot` — мобильный хотспот Windows, `public` — HTTPS-туннель). Не отправлять `lan_portal_url`, `hotspot_url`, `token` или другие адреса как отдельные поля без прямой просьбы.
...
**Личный кабинет / Mini App:** `/web`, «личный кабинет», «веб-план» → `node scripts/student-portal.mjs --user <user_key>`. Отправить пользователю **только** `portal_url` (LAN-адрес хоста агента, порт 18791). Напомнить по полю `mode`: `lan` — «подключи телефон к той же Wi‑Fi/сети, что и хост»; `hotspot` — «включи мобильный хотспот на ПК»; `public` — «открой ссылку в браузере». Не показывать `user_key`, telegram id, token или пути. Только при `setup_status: complete`. Если портал не запущен — «попроси владельца запустить кабинет».
