...
Учебный веб-интерфейс для **одного** пользователя: план на сегодня, Kanban/календарь и чат с ботом. Основной вариант — **хотспот ПК** (`http://192.168.137.1:18791/my/…`): телефон подключается к Wi‑Fi, который раздаёт компьютер.
...
Порт **18791**, слушает `0.0.0.0`. Доступ с телефона через **мобильный хотспот Windows** на этом ПК (`192.168.137.1`).
