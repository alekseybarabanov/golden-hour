...
```
> materials add --type problem --goal g_ege_math_profile
  Найдите все значения параметра a, при которых уравнение x² + ax + 1 = 0
  имеет два различных корня, оба меньше 2.
→ создаст materials/g_ege_math_profile/problems/2026-06-18_parametry.md (status: new)
→ допишет в memory/notes.jsonl
→ допишет в memory/2026-06-18.md

> materials pick g_ege_math_profile --tag параметры --status new
→ выдаст задачу, статус → working
+ inline buttons: [✅ Разобрала] [❌ Не поняла] [⏭ Пропустить]

> materials status m_a1b2c3d4 understood
→ обновит статус + запишет в историю
→ допишет в memory/2026-06-18.md (не в notes.jsonl)

> [кнопка] mat:status:m_8a1f2c3d:understood
→ тот же эффект, что и status understood
→ короткий confirm: "Засчитано ✅"
```
