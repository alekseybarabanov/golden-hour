...
> Единый скилл интеграции с Google: Tasks — список для рук, Sheet — источник истины и дашборды, Calendar — дедлайны с временем и контрольные точки. Альтернатива локальным `tasks` + `longterm-stats` (Obsidian/`users/<key>/tasks.yaml` → Google).
...
Шаблоны вывода — **идентичны** `tasks` и `longterm-stats` (скопировать дословно из applied-версий).
...
Без изменений относительно `tasks` + `longterm-stats`:
...
|-------|-----------------|
| `tasks` | **optional** → логика дублируется в `google-task-hub` при миграции на Google |
| `longterm-stats` | Локальная статистика (`longterm-stats.mjs`). При подключении `google-task-hub` — дублирующая логика в Sheet |
| `checkins` | читает deadline/scheduled из Sheet вместо plan json |
...
3. «что горит» → те же таблицы что в `tasks`
