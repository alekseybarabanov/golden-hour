...
**Team-уведомления:** из `team-tasks.mjs` → `notifications.recipients[]` (не throttled).
...
3. Запись: `node scripts/checkin-record.mjs --user <key> --text "…"` → `progress.md` + streak
4. Опционально: `node scripts/temporal-kg.mjs checkin --user <key> ...`
5. Закрытая тема → `[x]` в progress + `temporal-kg milestone`
...
`daily-plan`, `tasks`, `timer`, `temporal-kg`, `team-tasks`, `cards`
