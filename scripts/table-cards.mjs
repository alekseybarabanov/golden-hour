#!/usr/bin/env node
// table-cards.mjs — render markdown tables as PNG via study-cards (dark|light theme from profile).
//
// Usage:
...
import { CARD_THEME, resolveCardTheme, RENDER_TABLE_JS } from "./lib/card-render.mjs";
...
function renderOne(table, meta, outDir, index, theme) {
  const chunks = chunkTable(table, 4200, { title: meta.title, subtitle: meta.subtitle });
  const pngs = [];
  for (let ci = 0; ci < chunks.length; ci++) {
    ...
    try {
      execSync(
        `node "${RENDER_TABLE_JS}" --source="${jsonPath}" --output-dir="${outDir}" --name="${pngName}" --theme=${theme}`,
...
const title = opts.title || "Таблица";
...
const cardTheme = resolveCardTheme(userKey);
...
if (isDryRun(opts)) {
  out({
    ...
    output_dir: relOut,
    card_theme: cardTheme,
    summary: `Найдено таблиц: ${tables.length}. Будут PNG (${cardTheme}) в ${relOut}.`,
  });
  process.exit(0);
}
...
tables.forEach((table, i) => {
  ...
  allPngs.push(...renderOne(table, { title: tableTitle, subtitle }, outDir, i, cardTheme));
});
...
out({
  ...
  summary: `Готово: ${allPngs.length} PNG (${cardTheme}). Отправь картинками в Telegram, не markdown-таблицей.`,
});