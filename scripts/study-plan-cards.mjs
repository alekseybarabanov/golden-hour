...
import { resolveCardTheme, RENDER_ORCHESTRATOR } from "./lib/card-render.mjs";
...
if (isDryRun(opts)) {
  out({
    ...
    summary: `Карточки: обложка + ${cardPlan.weeks.length} тем (~${pngCount} PNG, ${cardTheme}).`,
  });
  process.exit(0);
}
...
try {
  execSync(
    `node "${RENDER_ORCHESTRATOR}" --mode=from-plan-file --source="${planJsonPath}" --output-dir="${cardsDir}" --themes=${cardTheme}`,
...
out({
  ...
  card_theme: cardTheme,
  png_files: pngs,
  summary: `Готово: ${pngs.length} карточек (${cardTheme}). Отправь альбомом в Telegram.`,
});
