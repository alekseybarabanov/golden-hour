...
Репозиторий агента: см. [INSTALL.md](../INSTALL.md).
...
10. [Доступ через хотспот ПК](#10-доступ-через-хотспот-пк)
...
| **cloudflared** (опционально) | HTTPS-кнопка в Telegram; иначе — браузер на хотспоте ПК |
...
См. [раздел 10](#10-доступ-через-хотспот-пк).
...
## 10. Доступ через хотспот ПК

По умолчанию всё работает **на этом компьютере**. Телефон подключается к **мобильному хотспоту Windows** на ПК.

1. Запустите порталы: `.\start_all_portals.ps1` или `.\repair-portals.ps1`
2. Windows → Параметры → Сеть → **Мобильный хотспот** → Вкл.
3. На телефоне подключитесь к Wi‑Fi этого ПК
4. Откройте в браузере:
   - учебный кабинет: `http://192.168.137.1:18791/my/<token>`
   - mini app: `http://192.168.137.1:18791/miniapp`

**Кнопка меню в Telegram** требует HTTPS. Для неё на ПК (не на VPS):

```powershell
.\repair-portals.ps1 -WithTunnels
# или
.\setup_telegram_miniapp.ps1
```

Локальный cloudflared даст временный `https://….trycloudflare.com`. Для постоянного домена — свой nginx + Let's Encrypt на ПК или домашнем сервере.
